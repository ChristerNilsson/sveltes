...
var app = (function () {
    ...
    function create_if_block_2(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			attr_dev(rect, "x", ctx.x-SIZE/2);
    			attr_dev(rect, "y", ctx.y-SIZE/2);
    			attr_dev(rect, "width", SIZE-1);
    			attr_dev(rect, "height", SIZE-1);
    			set_style(rect, "fill", (ctx.card.state==0 ? 'red' : 'green'));
    			add_location(rect, file, 22, 1, 382);
    			dispose = listen_dev(rect, "click", ctx.click_handler);
    		},
    	...
    	dispatch_dev("SvelteRegisterBlock", { block, id: create_if_block_2.name, type: "if", source: "(22:0) {#if card.state != 2}", ctx });
    	return block;
    }

    // (33:0) {#if card.state==1}
    function create_if_block(ctx) {
    	var if_block_anchor;

    	function select_block_type(changed, ctx) {
    		if (ctx.card.marker > 0) return create_if_block_1;
    	...
    	dispatch_dev("SvelteRegisterBlock", { block, id: create_if_block.name, type: "if", source: "(33:0) {#if card.state==1}", ctx });
    	return block;
    }

    // (40:1) {:else}
    function create_else_block(ctx) {
    	var image, image_href_value, dispose;

    	const block = {
    		c: function create() {
    			image = svg_element("image");
    			attr_dev(image, "class", "middle svelte-1vm1395");
    			attr_dev(image, "preserveAspectRatio", "");
    			attr_dev(image, "x", ctx.x-SIZE/2);
    			attr_dev(image, "y", ctx.y-SIZE/2);
    			attr_dev(image, "width", SIZE-1);
    			attr_dev(image, "height", SIZE-1);
    			attr_dev(image, "href", image_href_value = ctx.card.url);
    			add_location(image, file, 40, 2, 721);
    			dispose = listen_dev(image, "click", ctx.click_handler_2);
    		},
    		...
    		p: function update(changed, ctx) {
    			if ((changed.card) && image_href_value !== (image_href_value = ctx.card.url)) {
    	...
    	dispatch_dev("SvelteRegisterBlock", { block, id: create_else_block.name, type: "else", source: "(40:1) {:else}", ctx });
    	return block;
    }

    // (34:1) {#if card.marker > 0}
    function create_if_block_1(ctx) {
    	var text_1, t_value = ctx.card.text + "", t, dispose;

    	const block = {
    		c: function create() {
    			text_1 = svg_element("text");
    			t = text(t_value);
    			attr_dev(text_1, "class", "middle svelte-1vm1395");
    			attr_dev(text_1, "x", ctx.x);
    			attr_dev(text_1, "y", ctx.y);
    			add_location(text_1, file, 34, 2, 612);
    		...
    		p: function update(changed, ctx) {
    			if ((changed.card) && t_value !== (t_value = ctx.card.text + "")) {
    	...
    	dispatch_dev("SvelteRegisterBlock", { block, id: create_if_block_1.name, type: "if", source: "(34:1) {#if card.marker > 0}", ctx });
    	return block;
    }
    ...
    function create_fragment(ctx) {
    	...
    	var if_block0 = (ctx.card.state != 2) && create_if_block_2(ctx);

    	var if_block1 = (ctx.card.state==1) && create_if_block(ctx);
    ...
    class Card extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init(this, options, instance, create_fragment, safe_not_equal, ["index", "card", "click"]);
    		...
    		if (ctx.click === undefined && !('click' in props)) {
    			console.warn("<Card> was created without expected prop 'click'");
    		}
    	}
    ...
    var Symbol$1 = root$1.Symbol,
        iteratorSymbol$1 = Symbol$1 ? Symbol$1.iterator : undefined,
    ...
    function sampleSize$1(collection, n, guard) {
      ...
      if ((guard ? isIterateeCall$1(collection, n, guard) : n === undefined)) {
        ...
        n = baseClamp$1(toInteger$1(n), 0, length);
      }
      while (++index < n) {
    ...
    function isString$1(value) {
      return typeof value == 'string' ||
        (!isArray$1(value) && isObjectLike$1(value) && objectToString$1.call(value) == stringTag$1);
    }
    ...
    function toArray$1(value) {
      ...
      if (iteratorSymbol$1 && value[iteratorSymbol$1]) {
        return iteratorToArray$1(value[iteratorSymbol$1]());
      }
      var tag = getTag$1(value),
          func = tag == mapTag$1 ? mapToArray$1 : (tag == setTag$1 ? setToArray$1 : values$1);
    ...
    var lodash_samplesize = sampleSize$1;
    ...
    function create_each_block(ctx) {
    	...
    	const block = {
    		...
    		p: function update(changed, ctx) {
    			var card_changes = {};
    			if (changed.cards) card_changes.card = ctx.card;
    	...
    	dispatch_dev("SvelteRegisterBlock", { block, id: create_each_block.name, type: "each", source: "(62:1) {#each cards as card, index}", ctx });
    	return block;
    }
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(svg, file$1, 60, 0, 7746);
    		},
    		...
    		p: function update(changed, ctx) {
    			if (changed.cards || changed.click) {
    ...
    function instance$1($$self, $$props, $$invalidate) {
    	...
    	pairs = lodash_samplesize(pairs,N);

    	let cards = [];
    	for (const i of lodash_range(pairs.length)) {
    		const pair = pairs[i];
    		cards.push({marker: i+1, text:pair.text, state:0, url:pair.url});
    		cards.push({marker:-i-1, text:pair.text, state:0, url:pair.url});
    	}
    	$$invalidate('cards', cards = lodash_shuffle(cards));
    	...
    	const click = (card) => {
    		...
    		$$invalidate('cards', cards);
    		visible.push(card);
    		if (done>=2*N-2 && visible.length==2) $$invalidate('cards', cards=[]);
    	};
    	...
    	$$self.$inject_state = $$props => {
    		if ('pairs' in $$props) pairs = $$props.pairs;
